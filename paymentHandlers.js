...
module.exports = {
    // 1. ADD FUND par click karte hi UPI aur USDT options
    ...
    // UPI - PAYMENT COMPLETE trigger screen
    ...
    // USDT - NETWORK SELECT BUTTONS
    handleUsdtNetworkSelect: async (ctx) => {
        const parts = ctx.callbackQuery.data.split("_");
        const network = parts; // bep20 ya trc20
        const u = m.getOrCreateUser(ctx.from.id);
    ...
    // USDT - CONFIRM PAYMENT trigger screen
    handleUsdtConfirmClick: async (ctx) => {
        ...
        await ctx.editMessageText(
            ...
            `Bhai, apni USDT Transaction Hash ID niche message box mein type karke send karo aur sath mein transfer ka screenshot bhi attach karke bhejo:`,
            { parse_mode: "Markdown" }
        );
    },

    // Admin commands handling logic
    handleAdminActions: async (ctx) => {
        ...
        if (action === "acc") {
            ...
            await ctx.api.sendMessage(userId, successMsg, { parse_mode: "Markdown" }); 
    ...
    // TEXT INPUT RECEIVER SYSTEM (UPI Apps Grid aur USDT Multi-Network Routing)
