...
module.exports = {
    ...
    handleUsdtNetworkSelect: async (ctx) => {
        ...
        const address = network === "trc20" ? config.USDT_TRC20 : config.USDT_BEP20, netName = network.toUpperCase();
        const kb = new InlineKeyboard().text("CONFIRM PAYMENT", "usdt_confirm_click").row().text("BACK", "pay_via_usdt");
        await ctx.editMessageText(`🪙 *USDT ${netName} MANUAL DEPOSIT*\n\n💵 *Amount to Pay:* $${u.current_deposit_amt.toFixed(2)}\n📍 *${netName} Address:* \`${address}\`\n\n👉 *Instructions:* Diye gaye address par exactly $${u.current_deposit_amt.toFixed(2)} send karein aur uske baad neeche diye gaye *CONFIRM PAYMENT* button par click karein.`, { reply_markup: kb, parse_mode: "Markdown" });
    },
    handleUsdtConfirmClick: async (ctx) => {
        const u = m.getOrCreateUser(ctx.from.id); u.awaiting_utr = true;
        const orderNum = Math.floor(100000 + Math.random() * 900000); u.current_order_num = orderNum;
        await ctx.editMessageText(`🪙 *USDT Deposit Initiated!* ✅\n\n📊 *Requested Amount:* \`$${u.current_deposit_amt.toFixed(2)}\`\n🌐 *Network:* \`${u.chosen_network.toUpperCase()}\`\n\n⚠️ *SUBMIT TRANSACTION ID:*\nBhai, apni USDT Transaction Hash ID niche message box mein type karke send karo aur sath mein transfer ka screenshot bhi attach karke bhejo:`, { parse_mode: "Markdown" });
    }
};
