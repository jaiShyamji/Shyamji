...
// 🌟 आपके बताए अनुसार 9 बटन्स का बिल्कुल परफेक्ट लेआउट
...
module.exports = {
    ...
    backMenu: async (ctx) => { 
        const u = getOrCreateUser(ctx.from.id, ctx.from.first_name); u.awaiting_deposit_amt = false; 
    ...
    ordersHistory: async (ctx) => {
        ...
        if (u.history.length === 0) {
            txt += "▫️ No orders placed yet.";
        } else {
            u.history.slice(-5).forEach(o => { txt += `🆔 ID: \`${o.order_id}\` | Qty: ${o.qty} | Status: ${o.status}\n`; });
        }
        await ctx.editMessageText(txt, { reply_markup: new InlineKeyboard().text("⬅️ Back to Menu", "back_to_menu"), parse_mode: "Markdown" });
    },
    ...
    handleTextMessages: async (ctx) => {
        ...
        if (u.awaiting_deposit_amt && u.chosen_pay_method) {
            ...
            u.total_deposit_usd += (u.chosen_pay_method === "pay_via_upi" ? (amt / config.USD_TO_INR_RATE) : amt);
            return;
        }
