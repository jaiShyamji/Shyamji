...
module.exports = (bot) => {
    ...
    bot.command("addbalance", async (ctx) => {
        ...
        await ctx.reply(`💰 Added $${amt.toFixed(2)} to ${target}. New: $${core.DYNAMIC_USER_DB.users[target].balance_usd.toFixed(2)}`);
    ...
    bot.command("checkuser", async (ctx) => {
        if (ctx.from.id !== config.ADMIN_ID) return;
        const args = ctx.message.text.split(" "); const target = parseInt(args[1]); 
        if (isNaN(target) || !core.DYNAMIC_USER_DB.users[target]) return ctx.reply("❌ User nahi mila! ID check karein.");
    ...
    bot.on("message:text", async (ctx) => {
        ...
        if (u.awaiting_deposit_amt && u.chosen_pay_method) {
            ...
            if (u.chosen_pay_method === "pay_via_upi") {
                ...
                // 🪙 USDT STRING RENDER FIXED PERMANENTLY HERE BYHAI
        ...
        if (u.awaiting_utr) {
            ...
            await ctx.reply("💌 *Details Received!* ✅\n\nTnhara Reference/Transaction ID `" + txt + "` verification ke liye admin ke paas bhej diya gaya hai!"); return;
        }
