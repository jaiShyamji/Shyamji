...
let DYNAMIC_USER_DB = {};
const UNIFIED_DEPOSITS_MAP = {}; // Unified global sync container for accept/cancel fixes

// Auto-Load JSON Database Backup
...
// 👑 BOT COMMANDS & CALLBACKS LINKAGE
...
// 👑 100% FIXED SHARED-MEMORY APPROVAL ENGINES (No More Request Expired!)
bot.callbackQuery(/^adm_(acc|can)_(.+)_(.+)$/, async (ctx) => {
    ...
    const depositData = UNIFIED_DEPOSITS_MAP[refKey]; 
    if (!depositData) return ctx.answerCallbackQuery({ text: "❌ Link Broken or Expired!", show_alert: true });
    ...
    } delete UNIFIED_DEPOSITS_MAP[refKey];
});

// 👑 HIGH PRIORITY SUPER ADMIN COMMANDS INTERCEPTORS
