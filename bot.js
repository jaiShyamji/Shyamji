...
// 👑 ADMIN COMMANDS DIRECT ON THE ROOT FOR HIGHEST PRIORITY
...
bot.command("addbalance", async (ctx) => {
    if (ctx.from.id !== config.ADMIN_ID) return;
    const txt = ctx.message.text.replace("/addbalance", "").trim().split(" "), target = parseInt(txt), amt = parseFloat(txt);
...
bot.command("deductbalance", async (ctx) => {
    if (ctx.from.id !== config.ADMIN_ID) return;
    const txt = ctx.message.text.replace("/deductbalance", "").trim().split(" "), target = parseInt(txt), amt = parseFloat(txt);
...
bot.command("addservice", async (ctx) => {
    ...
    SERVICES_MASTER_DATA[args] = { name: args.slice(3).join(" "), rate: parseFloat(args), type: args }; fs.writeFileSync('./services.js', `module.exports = ${JSON.stringify(SERVICES_MASTER_DATA, null, 4)};`, 'utf-8'); await ctx.reply(`✅ Service Added successfully!`);
});
...
// Normal platforms routes integration
