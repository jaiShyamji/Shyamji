require('dotenv').config();const { Bot, InlineKeyboard } = require('grammy');const axios = require('axios');
const config = require('./config');const SERVICES_MASTER_DATA = require('./services');
if (!config.BOT_TOKEN) { console.error("Missing Token!"); process.exit(1); }
const bot = new Bot(config.BOT_TOKEN);const USER_DATABASE = {};
const TG_POST_RE = /https:\/\/t\.me\/([A-Za-z0-9_]+)\/(\d+)\/?/, TG_CHANNEL_RE = /https:\/\/t\.me\/([A-Za-z0-9_]+)\/?/;
const IG_POST_RE = /https:\/\/(www\.)?instagram\.com\/(?:p|reel|reels|tv)\/([A-Za-z0-9_\-]+)\/?/, IG_PROFILE_RE = /https:\/\/(www\.)?instagram\.com\/([A-Za-z0-9_\.]+)\/?/;
...
bot.command("start", async (ctx) => { const u = getOrCreateUser(ctx.from.id); await ctx.reply(`👋 Welcome to SMM Panel Bot!\n\n💳 Balance: ${formatMoney(u.balance_usd, u.currency)}`, { reply_markup: getMainMenuKeyboard() }); });
...
bot.hears(/^\/\d+$/, async (ctx) => { const id = ctx.message.text.slice(1); if (!SERVICES_MASTER_DATA[id]) return; const u = getOrCreateUser(ctx.from.id); u.pending_service = id; u.awaiting_custom_qty = false; await ctx.reply(`👉 *You selected:* ${SERVICES_MASTER_DATA[id].name}\n\n🔢 *Select Your Quantity:*`, { reply_markup: getQuantityKeyboard(id), parse_mode: "Markdown" }); });
...
bot.on("message:text", async (ctx) => {
    ...
    if (u.awaiting_deposit_amt && u.chosen_pay_method) {
        const amt = parseFloat(ctx.message.text.trim()); if (isNaN(amt) || amt <= 0) { await ctx.reply("❌ Invalid amount. Try again:"); return; }
        u.awaiting_deposit_amt = false; const kb = new InlineKeyboard().text("✅ मैंने पेमेंट कर दी है", "payment_done_proof").row().text("⬅️ Main Menu", "back_to_menu");
        if (u.chosen_pay_method === "pay_via_upi") {
            ...
            await ctx.replyWithPhoto(qrUrl, { caption: `🟢 *UPI Automatic QR Code*\n\n💵 *Amount:* ₹${amt.toFixed(2)}\n📍 *UPI ID:* \`${config.UPI_ID}\`\n\n⚠️ स्कैन करके पे करें और स्क्रीनशॉट सपोर्ट पर भेजें।`, reply_markup: kb, parse_mode: "Markdown" });
        } else {
            const qrUrl = `https://googleapis.com{encodeURIComponent(config.USDT_ADDRESS)}`;
            await ctx.replyWithPhoto(qrUrl, { caption: `🪙 *USDT (TRC20) QR Code*\n\n💵 *Amount:* $${amt.toFixed(2)}\n📍 *Address:* \`${config.USDT_ADDRESS}\`\n\n⚠️ इस एड्रेस पर ट्रांसफर करके स्क्रीनशॉट सपोर्ट पर भेजें।`, reply_markup: kb, parse_mode: "Markdown" });
        } return;
    }
    if (u.awaiting_custom_qty && u.pending_service) { const q = parseInt(ctx.message.text.trim()); if (isNaN(q) || q <= 0) { await ctx.reply("❌ Invalid number:"); return; } u.awaiting_custom_qty = false; await proceedToLinkRequest(ctx, u, u.pending_service, q); return; }
    if (!u.pending_service || !u.pending_qty) return;
    const id = u.pending_service, link = ctx.message.text.trim(), sInfo = SERVICES_MASTER_DATA[id];
    if (!validateLink(link, sInfo.type)) { await ctx.reply("❌ *Invalid Link Format!*"); return; }
    if (u.balance_usd < u.pending_cost_usd) { await ctx.reply(`❌ *Insufficient Balance!*`); u.pending_service = null; return; }
    try {
        const res = await axios.post(config.SMM_API_URL, null, { params: { key: config.SMM_API_KEY, action: "add", service: id, link: link, quantity: u.pending_qty }, timeout: 20000 });
        if (res.data && res.data.order) {
            u.balance_usd -= u.pending_cost_usd; u.spent_usd += u.pending_cost_usd; u.orders_count++;
