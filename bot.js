require('dotenv').config();const { Bot, InlineKeyboard } = require('grammy');const axios = require('axios');const config = require('./config');const SERVICES_MASTER_DATA = require('./services');
if (!config.BOT_TOKEN) process.exit(1);const bot = new Bot(config.BOT_TOKEN);const USER_DATABASE = {};
const TG_POST_RE = /https:\/\/t\.me\/([A-Za-z0-9_]+)\/(\d+)\/?/, TG_CHANNEL_RE = /https:\/\/t\.me\/([A-Za-z0-9_]+)\/?/, IG_POST_RE = /https:\/\/(www\.)?instagram\.com\/(?:p|reel|reels|tv)\/([A-Za-z0-9_\-]+)\/?/, IG_PROFILE_RE = /https:\/\/(www\.)?instagram\.com\/([A-Za-z0-9_\.]+)\/?/;
function getOrCreateUser(id) { if (!USER_DATABASE[id]) USER_DATABASE[id] = { balance_usd: 0.0, spent_usd: 0.0, orders_count: 0, history: [], currency: "INR", pending_service: null, pending_qty: null, pending_cost_usd: null, awaiting_custom_qty: false, awaiting_deposit_amt: false, chosen_pay_method: null }; return USER_DATABASE[id]; }
function formatMoney(usd, pref) { return pref === "INR" ? `₹${(usd * config.USD_TO_INR_RATE).toFixed(2)}` : `$${usd.toFixed(2)}`; }
...
bot.callbackQuery("back_to_menu", async (ctx) => { const u = getOrCreateUser(ctx.from.id); u.awaiting_deposit_amt = false; await ctx.editMessageText(`👋 Main Menu\n\n💳 Balance: ${formatMoney(u.balance_usd, u.currency)}`, { reply_markup: getMainMenuKeyboard() }); });
bot.callbackQuery("main_services", async (ctx) => { await ctx.editMessageText(`🛠️ *Select Platform:*`, { reply_markup: getPlatformsKeyboard(), parse_mode: "Markdown" }); });
...
bot.callbackQuery(/^q_\d+_(.+)$/, async (ctx) => {
    ...
    const sInfo = SERVICES_MASTER_DATA[serviceId]; u.pending_qty = parseInt(qtyType); u.pending_cost_usd = (u.pending_qty / 1000.0) * sInfo.rate;
    await ctx.editMessageText(`📋 *Summary*\n🛠️ Service: \`${sInfo.name}\`\n📊 Qty: \`${u.pending_qty}\`\n💸 Cost: ${formatMoney(u.pending_cost_usd, u.currency)}\n\n🔗 Send Target Link:`, { parse_mode: "Markdown" });
});
bot.callbackQuery("main_add_funds", async (ctx) => { const kb = new InlineKeyboard().text("🇮🇳 UPI", "pay_via_upi").text("🪙 USDT", "pay_via_usdt").row().text("⬅️ Back", "back_to_menu"); await ctx.editMessageText("💳 *Select Payment Method:*", { reply_markup: kb, parse_mode: "Markdown" }); });
bot.callbackQuery(/^pay_(via_upi|via_usdt)$/, async (ctx) => { const u = getOrCreateUser(ctx.from.id); u.chosen_pay_method = ctx.callbackQuery.data; u.awaiting_deposit_amt = true; await ctx.editMessageText(`💰 *Enter Amount:* ` + (u.chosen_pay_method === "pay_via_upi" ? "Type INR ₹ Amount:" : "Type USD $ Amount:")); });
bot.on("message:text", async (ctx) => {
    ...
    if (u.awaiting_deposit_amt && u.chosen_pay_method) {
        ...
        if (u.chosen_pay_method === "pay_via_upi") {
            const upiUrl = `upi://pay?pa=${config.UPI_ID}&pn=${encodeURIComponent(config.MERCHANT_NAME)}&am=${amt.toFixed(2)}&cu=INR`, qrUrl = `https://googleapis.com{encodeURIComponent(upiUrl)}`;
    ...
    if (u.awaiting_custom_qty && u.pending_service) { const q = parseInt(txt), sInfo = SERVICES_MASTER_DATA[u.pending_service]; if (isNaN(q) || q <= 0) { await ctx.reply("❌ Invalid number:"); return; } u.awaiting_custom_qty = false; u.pending_qty = q; u.pending_cost_usd = (q / 1000.0) * sInfo.rate; await ctx.reply(`📋 *Summary*\n🛠️ Service: \`${sInfo.name}\`\n📊 Qty: \`${q}\`\n💸 Cost: ${formatMoney(u.pending_cost_usd, u.currency)}\n\n🔗 Send Target Link:`, { parse_mode: "Markdown" }); return; }
...
bot.callbackQuery("p_done", async (ctx) => { await ctx.reply(`💌 *Registered!* Send screenshot to @${config.SUPPORT_USERNAME}`); });
bot.callbackQuery("main_orders", async (ctx) => {
    const u = getOrCreateUser(ctx.from.id); let txt = `📦 *Order History:*\n\nTotal: ${u.orders_count}\n\n`;
