...
// Dono engines ko sequential load par lock kiya bhai
require('./combinedEngine')(bot);

async function startBotEngine() {
    try { await bot.api.deleteWebhook({ drop_pending_updates: true }); run(bot); console.log("HAPPY REACTION Engine Live!"); } catch (err) { setTimeout(startBotEngine, 5000); }
}
startBotEngine();
