...
module.exports = (bot) => {
    ...
    bot.command("ban", async (ctx) => {
        ...
        if (isNaN(target) || !core.DYNAMIC_USER_DB.users[target]) return ctx.reply("❌ ID nahi mili!");
    ...
    bot.command("addbalance", async (ctx) => {
        if (ctx.from.id !== config.ADMIN_ID) return;
        const txt = ctx.message.text.replace("/addbalance", "").trim().split(" ");
        const target = parseInt(txt[0]), amt = parseFloat(txt[1]);
    ...
    bot.command("deductbalance", async (ctx) => {
        if (ctx.from.id !== config.ADMIN_ID) return;
        const txt = ctx.message.text.replace("/deductbalance", "").trim().split(" ");
        const target = parseInt(txt[0]), amt = parseFloat(txt[1]);
    ...
    bot.command("checkuser", async (ctx) => {
        ...
        if (isNaN(target) || !core.DYNAMIC_USER_DB.users[target]) return ctx.reply("❌ User nahi mila!");
    ...
    bot.command("updateservice", async (ctx) => {
        if (ctx.from.id !== config.ADMIN_ID) return;
        const args = ctx.message.text.replace("/updateservice", "").trim().split(" ");
        const id = args[0], newRate = parseFloat(args[1]); if (!SERVICES_MASTER_DATA[id] || isNaN(newRate)) return ctx.reply("❌ Not found!");
